...
export const signup = async(req, res) => {
    ...
    try {
        ...
        const result = await customer.create({ email, password: encryptedPassword, fullname, phone });
        const token = jwt.sign({ fullname: result.fullname, email: result.email, id: result._id, phone: result.phone}, 'test', { expiresIn: "30d" });
